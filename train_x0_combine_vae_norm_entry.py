...
def train(rank, gpu, args):
    ...
    if args.dataset in ['cifar10', 'mnist', 'stackmnist', 'stl10', 'celeba_64']:
        print("using tiny discriminator")   
        netD = Discriminator_tiny(nc = 2*args.num_channels, ngf = args.ngf,
    ...
    algo = 'uot_combine_vae_norm_entry'
    ...
    lazy_potential = args.lazy_potential
    
    for epoch in range(init_epoch, args.num_epoch+1):
        train_sampler.set_epoch(epoch)
       
        for iteration, (real_data, y) in enumerate(tqdm(data_loader)):
            ...
            with torch.no_grad():
                real_data = vae.encode(real_data).latent_dist.sample().mul_(0.18215)
            real_data = real_data - mean_data
            real_data = real_data / std_data
            step_weight_type = torch.tensor(args.step_weight, device=device)

            if iteration % lazy_potential == 0:
                #sample t
                t = torch.randint(0, args.num_timesteps, (real_data.size(0),), device=device)
                step_weight = step_weight_type[t]
                ...
                D_real = netD(real_data, t, x_tp1.detach())
                # errD_real = phi_star1(loss_weight*(D_real - upper_d) - loss_weight)
                ...
                D_fake = netD(x_0_predict, t, x_tp1.detach())
                # errD_fake = phi_star2(-loss_weight*D_fake - loss_weight)
                # errD_fake = errD_fake.mean()
                # errD_fake.backward(retain_graph=True)

                errD_regularization = phi_star1(D_fake - tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)) + phi_star2(-D_real)
                errD_regularization = (step_weight * errD_regularization).mean()
                # errD_regularization.backward(retain_graph=True)

                errD = D_fake - tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_real
                errD = (step_weight * errD).mean()
                # errD.backward(retain_graph=True)

                (errD * (1 - loss_weight) + errD_regularization * loss_weight).backward(retain_graph=True)
                ...
                if global_step % args.lazy_reg == 0:
                    ...
                    grad_penalty = (step_weight * (grad_real.view(grad_real.size(0), -1).norm(2, dim=1) ** 2)).mean()
            ...
            errG = tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_fake
            errG = (step_weight * errG).mean()
            ...
            if iteration % 100 == 0:
                if rank == 0:
                    loss_content = 'epoch {} iteration {}, G Loss: {}, D Loss: {}, D UOTLoss: {}'.format(epoch,iteration, errG.item(), errD.item(), errD_regularization.item())
        ...
        if rank == 0:
            if epoch % 5 == 0:
                with torch.no_grad():
                    ...
                    del real_data
                    x_t_1 = torch.randn((8, num_channels, input_size, input_size), device='cuda')
                    fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args)
                    fake_sample = fake_sample * std_data
                    fake_sample = fake_sample + mean_data
                    fake_sample = vae.decode(fake_sample / 0.18215).sample
