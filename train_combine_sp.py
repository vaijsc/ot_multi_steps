...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            errD_regularization = phi_star1(D_fake - tausp * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)) + phi_star2(-D_real)
            ...
            errG_regularization = tausp * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_fake
            errG_regularization = errG_regularization.mean()
            errG = tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_fake
            errG = errG.mean()
            
            (errG * (1 - loss_weight) + errG_regularization * loss_weight).backward()
            optimizerG.step()
