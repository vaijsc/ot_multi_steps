...
def train(rank, gpu, args):
    ...
    algo = 'uot_xt_detach_sp_potential'
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            # train with real
            D_real = netD(x_t, t, real_data.detach())
            ...
            output = netD(x_pos_sample, t, real_data.detach())
                
            errD_fake = output - args.tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)
            # errD_fake = errD_fake.mean()
            # errD_fake.backward()
    
            
            errD = phi_star1(errD_real + errD_fake).mean()
            ...
            errG = args.tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - output
            errG = phi_star2(errG).mean()
