...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            errD_fake = output - args.tau * torch.sum(((x_pos_sample-x_tp1_3.detach()).view(x_tp1_3.detach().size(0), -1))**2, dim=1)
            ...
            errG = errG.mean()
            errG_absolute = torch.abs(errG.detach()) + 1e-6
            
            (errG / errG_absolute).backward()
            optimizerG.step()
