...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            P_real = netP(real_data, t, x_tp1.detach())
            ...
            P_fake = netP(x_0_predict, t, x_tp1.detach())

            # softplus
            errP_regularization = phi_star1(P_fake - tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)) + phi_star2(-P_real)
            errP_regularization = errP_regularization.mean()

            # linear
            errP = P_fake - tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - P_real
            errP = errP.mean()
            ...
            if global_step % args.lazy_reg == 0:
                ...
                grad_real = torch.autograd.grad(
                                outputs=P_real.sum(), inputs=real_data, create_graph=True
                                )[0]
            ...
            errG = tau * torch.sum(((x_0_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - P_fake
