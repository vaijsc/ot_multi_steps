...
def train(rank, gpu, args):
    ...
    algo = 'uot_xt_detach_r2_both_pt3_bound'
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            x_t_2, x_tp1_2, _ = q_sample_pairs(coeff, real_data, t)
            x_t_3, x_tp1_3, _ = q_sample_pairs(coeff, real_data, t)
            # import ipdb; ipdb.set_trace()
    
            # train with real
            x_t_2.requires_grad = True
            D_real = netD(x_t_2, t, x_tp1.detach())
            errD_real = phi_star1(-5 * (D_real + upper_d) - 5) + phi_star2(5 * (D_real - upper_d) - 5)
            # errD_real = errD_real.mean()
            errD_real = (loss_weight * errD_real).mean()
            ...
            if global_step % args.lazy_reg == 0:
                ...
                grad_real = torch.autograd.grad(
                                outputs=D_real.sum(), inputs=x_t_2, create_graph=True
                                )[0]
            ...
            errD = D_fake - tau * torch.sum(((x_pos_sample-x_tp1_3.detach()).view(x_tp1_3.detach().size(0), -1))**2, dim=1) - D_real
            # errD = (loss_weight * errD).mean()
            errD = errD.mean()
            errD.backward()
            ...
            errG = tau * torch.sum(((x_pos_sample-x_tp1_3.detach()).view(x_tp1_3.detach().size(0), -1))**2, dim=1) - D_fake
            errG = errG.mean()
