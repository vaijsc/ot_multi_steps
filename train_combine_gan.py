...
def train(rank, gpu, args):
    ...
    if args.dataset in ['cifar10', 'mnist', 'stackmnist', 'stl10', 'celeba_64']:
        ...
        netP = Discriminator_small(nc = 2*args.num_channels, ngf = args.ngf,
        ...
        netP = Discriminator_large(nc = 2*args.num_channels, ngf = args.ngf, 
                                   t_emb_dim = args.t_emb_dim,
                                   act=nn.LeakyReLU(0.2)).to(device)
    ...
    broadcast_params(netP.parameters())
    broadcast_params(netD.parameters())
    
    optimizerP = optim.Adam(netP.parameters(), lr=args.lr_d, betas = (args.beta1, args.beta2))
    ...
    schedulerP = torch.optim.lr_scheduler.CosineAnnealingLR(optimizerP, args.schedule, eta_min=1e-5)
    ...
    netP = nn.parallel.DistributedDataParallel(netP, device_ids=[gpu])
    netD = nn.parallel.DistributedDataParallel(netD, device_ids=[gpu])

    
    algo = 'uot_combine_gan'
    ...
    if args.resume:
        ...
        # load P
        netP.load_state_dict(checkpoint['netP_dict'])
        optimizerP.load_state_dict(checkpoint['optimizerP'])
        schedulerP.load_state_dict(checkpoint['schedulerP'])
        # load D
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            P_real = netP(x_t, t, x_tp1.detach())
            D_real = netD(real_data, t, x_tp1.detach()).view(-1)
            ...
            x_0_predict = netG(x_tp1.detach(), t, latent_z).detach() # detach since not updating G
            x_pos_sample, _ = sample_posterior(pos_coeff, x_0_predict, x_tp1, t)
            x_pos_sample.requires_grad = True
            P_fake = netP(x_pos_sample, t, x_tp1.detach())

            # softplus
            errP_regularization = phi_star1(P_fake - tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)) + phi_star2(-P_real)
            errP_regularization = errP_regularization.mean()

            # linear
            errP = P_fake - tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - P_real
            errP = errP.mean()

            # Backward for weighted loss between softplus and linear
            (errP * (1 - loss_weight) + errP_regularization * loss_weight).backward(retain_graph=True)
            
            # Make D smooth
            if global_step % args.lazy_reg == 0:

                grad_real = torch.autograd.grad(
                                outputs=P_real.sum(), inputs=x_t, create_graph=True
                                )[0]
                ...
                grad_real = torch.autograd.grad(
                        outputs=D_real.sum(), inputs=real_data, create_graph=True
                        )[0]
                grad_penalty = (
            ...
            output = netD(x_0_predict, t, x_tp1.detach()).view(-1)                            
            ...
            P_fake = netP(x_pos_sample, t, x_tp1.detach())
            D_fake = netD(x_0_predict, t, x_tp1.detach()).view(-1)
               
            errG = tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - P_fake
            errG = errG.mean() + F.softplus(-D_fake).mean()
            ...
            if iteration % 100 == 0:
                if rank == 0:
                    loss_content = 'epoch {} iteration {}, G Loss: {}, P Loss: {}, D Loss: {}'.format(epoch,iteration, errG.item(), errP.item(), errD.item())
            ...
            if args.save_content:
                if epoch % args.save_content_every == 0:
                    ...
                    content = {'epoch': epoch + 1, 'global_step': global_step, 'args': args,
                               ...
                               'schedulerG': schedulerG.state_dict(), 'netP_dict': netP.state_dict(),
                               'optimizerP': optimizerP.state_dict(), 'schedulerP': schedulerP.state_dict(), 'netD_dict': netD.state_dict(),
