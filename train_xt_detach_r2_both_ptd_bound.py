...
from util.utility import copy_source, broadcast_params, q_sample_pairs, sample_posterior, sample_from_model, select_phi, q_sample_pairs_double_pos
...
def train(rank, gpu, args):
    ...
    algo = 'uot_xt_detach_r2_both_ptd_bound'
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            x_t, x_tp1, x_tp1_2 = q_sample_pairs_double_pos(coeff, real_data, t)
            
    
            # train with real
            x_t.requires_grad = True
            D_real = netD(x_t, t, x_tp1.detach())
            errD_real = phi_star1(-loss_weight * (D_real + upper_d) - loss_weight) + phi_star2(loss_weight * (D_real - upper_d) - loss_weight)
            ...
            errD = D_fake - tau * torch.sum(((x_pos_sample-x_tp1_2.detach()).view(x_tp1_2.detach().size(0), -1))**2, dim=1) - D_real
            # errD = (loss_weight * errD).mean()
            errD = errD.mean()
            errD.backward()
            ...
            errG = tau * torch.sum(((x_pos_sample-x_tp1_2.detach()).view(x_tp1_2.detach().size(0), -1))**2, dim=1) - D_fake
            errG = errG.mean()
