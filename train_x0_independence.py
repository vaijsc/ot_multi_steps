...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, ((x, y), (x2, y2)) in enumerate(tqdm(zip(data_loader, data_loader_duplicate), total=total_iterations)):
            ...
            # Require grads for regularization
            real_data.requires_grad = True
            x_t.requires_grad = True
            
    
            # train with real
            D_real = netD(real_data, t)
            ...
            output = netD(x_0_predict, t)
                
            errD_fake = phi_star1(output - args.tau * torch.sum(((x_0_predict-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1))
            ...
            errG = args.tau * torch.sum(((x_0_predict-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1) - output
            errG = errG.mean()
