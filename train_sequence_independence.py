...
from util.utility import copy_source, broadcast_params, q_sample_pairs, select_phi
...
def train(rank, gpu, args):
    ...
    algo = 'uot_sequence_independence'
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, ((x, y), (x2, y2)) in enumerate(tqdm(zip(data_loader, data_loader_duplicate))):
            ...
            x_t_predict = netG(x_tp1.detach(), t, latent_z)
            
            
            output = netD(x_t_predict, t)
                
            errD_fake = phi_star1(output - args.tau * torch.sum(((x_t_predict-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1))
            ...
            errG = args.tau * torch.sum(((x_t_predict-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1) - output
            errG = errG.mean()
