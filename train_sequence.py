...
from util.utility import copy_source, broadcast_params, q_sample_pairs, sample_posterior, select_phi
...
def train(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            errD_fake = phi_star1(output - args.tau * torch.sum(((x_t_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1))
            ...
            errG = args.tau * torch.sum(((x_t_predict-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - output
            errG = errG.mean()
