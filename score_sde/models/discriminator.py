...
class Discriminator_tiny(nn.Module):
  ...
  def __init__(self, nc = 3, ngf = 64, t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
    ...
    self.final_conv = conv2d(ngf*2 + 1, ngf*2, 3,padding=1, init_scale=0.)
    self.end_linear = dense(ngf*2, 1)
...
class Discriminator_64(nn.Module):
  ...
  def forward(self, x, t, x_t):
    ...
    h4 = self.conv4(h3,t_embed)
   
    
    out = self.conv5(h4,t_embed)
