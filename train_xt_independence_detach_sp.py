...
def train(rank, gpu, args):
    ...
    algo = 'uot_xt_independence_detach_sp'
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, ((x, y), (x2, y2)) in enumerate(tqdm(zip(data_loader, data_loader_duplicate), total=total_iterations)):
            ...
            output = netD(x_pos_sample, t)
                
            errD_fake = output - args.tau * torch.sum(((x_pos_sample-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1)
            ...
            errG = args.tau * torch.sum(((x_pos_sample-x_tp1_duplicate.detach()).view(x_tp1_duplicate.detach().size(0), -1))**2, dim=1) - output
