...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            x0 = generator(x, t_time, latent_z)
            x = x0.detach()
    return x
...
def sample_and_test(args, epoch_id):
    ...
    algo = args.algo
    # result_dir = f'./saved_info/{algo}/{args.dataset}'
    # save_dir = f'./generated_samples/{algo}/{args.dataset}'
    exp_path = f'./saved_info/{args.dataset}'
    samples_dir = f'./generated_samples/{args.dataset}'
    if args.perturb_percent > 0:
        exp_path += f'_{int(args.perturb_percent)}p_{args.perturb_dataset}'
        samples_dir += f'_{int(args.perturb_percent)}p_{args.perturb_dataset}'
    exp_path += f'/{algo}/{args.phi1}_{args.phi2}/tau_{args.tau}/bs{train_batch_size}/lrg_{args.lr_g}_lrd_{args.lr_d}/r1_gamma_{args.r1_gamma}/{args.exp}'
    samples_dir += f'/{algo}/{args.exp}'
    
    result_folder = exp_path + f'/stat'
    netG_path = f'{exp_path}/netG/netG_{epoch_id}.pth'
    ...
    if args.dataset == 'cifar10':
        ...
        real_img_dir = 'pytorch_fid/celeba_train_stat.npy'
    elif args.dataset == 'celebahq_256' or args.dataset == 'celeba_hq':
    ...
    json_filename = f'{result_folder}/fid.json'
    print(json_filename)


    
    netG = NCSNpp(args).to(device)
    ckpt = torch.load(netG_path, map_location=device)
    ...
    if args.compute_fid:
        for i in range(iters_needed):
            with torch.no_grad():
                ...
                for j, x in enumerate(fake_sample):
                    ...
                    torchvision.utils.save_image(x, f'{samples_dir}/{index}.jpg')
        ...
        data = dict(sorted(data.items(), key=lambda x: int(str(x[0]).split()[1])))
        with open(json_filename, 'w') as json_file:
            json.dump(data, json_file)
    else:
        x_t_1 = torch.randn(args.batch_size, args.num_channels,args.image_size, args.image_size).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', help='name of the training algorithm')
    ...
    parser.add_argument('--epoch_start', type=int,default=1000)
    parser.add_argument('--epoch_end', type=int,default=1000)
    parser.add_argument('--epoch_jump', type=int,default=25)
