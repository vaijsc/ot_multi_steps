    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (real_data, y) in enumerate(tqdm(data_loader)):
            ...
            if iteration % lazy_potential == 0:
                ...
                D_fake = netD(x_pos_sample, t, x_tp1.detach())
                # errD_fake = phi_star2(-loss_weight*D_fake - loss_weight)
                # errD_fake = errD_fake.mean()
                # errD_fake.backward(retain_graph=True)

                errD_regularization = phi_star1(D_fake - tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1)) + phi_star2(-D_real)
                errD_regularization = errD_regularization.mean()
                # errD_regularization.backward(retain_graph=True)

                errD = D_fake - tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_real
                errD = errD.mean()
            ...
            errG = tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - D_fake
            errG = errG.mean()
